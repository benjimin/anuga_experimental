...
Very shallow water flowing down a coarsely resolved slope, with drains
...
l0 = 10 # Length scale associated with triangle side length in channel (min_triangle area = 0.5*l0^2)
#l0 = 4
...
flow_algorithm = 'DE_SG' #'DE1' #'DE_SG'
reference_gradient_type = 'bed-slope' #'zero' #'bed-slope'

drain_count = 3
drains_proportion_of_width = 0.25
drains_proportion_of_length = 0.8
drain_depth = -3.0 #-0.000001
...
# (Although if it were frictionless, there could not be steady-uniform flow)
def frictionFun(x,y):
    return man_n + x*0.

def stageFun(x,y):
    out = topography(x,y) -100.0 # Dry
    return out
...
if __name__ == '__main__':
    ...
    domain.evolve_max_timestep = 1.0
    ...
    domain.set_quantity('elevation', topography, location='centroids') #, location='centroids')
    ...
    # GD NOTE: Might be too large a timestep on the first step?
    # No -- but it rapidly becomes unstable later on
    # domain.evolve_max_timestep = 0.01
    for t in domain.evolve(yieldstep=5.0, finaltime=100*60.0):

        print domain.time, domain.timestep, domain.edge_timestep.min()

        domain.report_water_volume_statistics()

        #domain.report_cells_with_small_local_timestep()
        if domain.flow_algorithm == "DE_SG":
            print 'Volume: ', domain.quantities['vol'].centroid_values.sum()
