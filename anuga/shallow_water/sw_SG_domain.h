...
// Adjusted for the subgrid case
//
// Stephen Roberts 2012
// Gareth Davies, 2014 - 2015
...
struct domain {
    ...
    double* alphax_edge_values;
    double* alphay_edge_values;

    double* stage_centroid_values;
    double* vol_centroid_values;
    double* u_vol_centroid_values;
    double* v_vol_centroid_values;
    double* xmom_centroid_values;
    double* ymom_centroid_values;
    double* bed_centroid_values;
    double* height_centroid_values;
    double* alphax_centroid_values;
    double* alphay_centroid_values;
    ...
    double* vol_explicit_update;
    double* u_vol_explicit_update;
    double* v_vol_explicit_update;
    double* xmom_explicit_update;
    double* ymom_explicit_update;
    
    //double* stage_semi_implicit_update;
    //double* vol_semi_implicit_update;
    double* u_vol_semi_implicit_update;
    double* v_vol_semi_implicit_update;
    ...
    double* subgrid_centroid_table;
    double* subgrid_edge_table;
    long* subgrid_centroid_starting_index;
    long* subgrid_edge_starting_index;
    long* subgrid_centroid_i_ncol;
    long* subgrid_edge_i_ncol;
    long* subgrid_centroid_last_lookup_col;
    long* subgrid_edge_last_lookup_col;
    double* subgrid_wet_area;
};
...
struct domain* get_python_domain(struct domain *D, PyObject *domain) {
    ...
    D->alphax_edge_values     = get_python_array_data_from_dict(quantities, "alphax",     "edge_values");
    D->alphay_edge_values     = get_python_array_data_from_dict(quantities, "alphay",     "edge_values");

    D->stage_centroid_values     = get_python_array_data_from_dict(quantities, "stage",     "centroid_values");
    D->vol_centroid_values     = get_python_array_data_from_dict(quantities, "vol",     "centroid_values");
    D->u_vol_centroid_values     = get_python_array_data_from_dict(quantities, "u_vol",     "centroid_values");
    D->v_vol_centroid_values     = get_python_array_data_from_dict(quantities, "v_vol",     "centroid_values");
    ...
    D->alphax_centroid_values     = get_python_array_data_from_dict(quantities, "alphax",     "centroid_values");
    D->alphay_centroid_values     = get_python_array_data_from_dict(quantities, "alphay",     "centroid_values");
    ...
    D->vol_explicit_update = get_python_array_data_from_dict(quantities, "vol",     "explicit_update");
    D->u_vol_explicit_update = get_python_array_data_from_dict(quantities, "u_vol",     "explicit_update");
    D->v_vol_explicit_update = get_python_array_data_from_dict(quantities, "v_vol",     "explicit_update");
    ...
    //D->vol_semi_implicit_update = get_python_array_data_from_dict(quantities, "vol",     "semi_implicit_update");
    D->u_vol_semi_implicit_update = get_python_array_data_from_dict(quantities, "u_vol",     "semi_implicit_update");
    D->v_vol_semi_implicit_update = get_python_array_data_from_dict(quantities, "v_vol",     "semi_implicit_update");
    ...
    subgrid_data = get_python_object(domain,"subgrid_data");

    subgrid_centroid_table = get_consecutive_array(subgrid_data, "subgrid_centroid_table");
    D->subgrid_centroid_table = (double*) subgrid_centroid_table->data;

    subgrid_edge_table = get_consecutive_array(subgrid_data, "subgrid_edge_table");
    D->subgrid_edge_table = (double*) subgrid_edge_table->data;
    
    subgrid_centroid_starting_index = get_consecutive_array(subgrid_data, "subgrid_centroid_starting_index");
    D->subgrid_centroid_starting_index = (long*) subgrid_centroid_starting_index->data;

    subgrid_edge_starting_index = get_consecutive_array(subgrid_data, "subgrid_edge_starting_index");
    D->subgrid_edge_starting_index = (long*) subgrid_edge_starting_index->data;
    
    subgrid_centroid_i_ncol = get_consecutive_array(subgrid_data, "subgrid_centroid_i_ncol");
    D->subgrid_centroid_i_ncol = (long*) subgrid_centroid_i_ncol->data;

    subgrid_edge_i_ncol = get_consecutive_array(subgrid_data, "subgrid_edge_i_ncol");
    D->subgrid_edge_i_ncol = (long*) subgrid_edge_i_ncol->data;
    
    subgrid_centroid_last_lookup_col = get_consecutive_array(subgrid_data, "subgrid_centroid_last_lookup_col");
    D->subgrid_centroid_last_lookup_col = (long*) subgrid_centroid_last_lookup_col->data;

    subgrid_edge_last_lookup_col = get_consecutive_array(subgrid_data, "subgrid_edge_last_lookup_col");
    D->subgrid_edge_last_lookup_col = (long*) subgrid_edge_last_lookup_col->data;

    subgrid_wet_area = get_consecutive_array(subgrid_data, "subgrid_wet_area");
    D->subgrid_wet_area = (double*) subgrid_wet_area->data;
    

    Py_DECREF(quantities);
    Py_DECREF(riverwallData);
    Py_DECREF(subgrid_data);
